package com.example.linedivideafterotview
...
fun Canvas.drawLineDivideAfterRot(scale : Float, w : Float, h : Float, paint : Paint) {
    val size : Float = Math.min(w, h) / sizeFactor
    val dsc : (Int) -> Float = { scale.divideScale(it, parts) }
    drawXY(w / 2, h / 2 - (h / 2) * dsc(4)) {
        rotate(rot * dsc(1))
        drawLineWithoutDot(0f, -size * 0.5f * dsc(2), 0f, size * 0.5f * dsc(2), paint)
        for (j in 0..1) {
            drawXY(0f, size * 0.5f * dsc(2) * (1f - 2 * j)) {
...
class LineDivideAfterRotView(ctx : Context) : View(ctx) {
    ...
    data class LDARNode(var i : Int = 0, val state : State = State()) {

        private var prev : LDARNode? = null
        private var next : LDARNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLDARNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LDARNode {
            var curr : LDARNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineDivideAfterRot(var i : Int) {

        private var curr : LDARNode = LDARNode(0)

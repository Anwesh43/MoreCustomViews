...
class LineBiSineWaveView(ctx : Context) : View(ctx) {
    ...
    data class LBSWNode(var i : Int = 0, val state : State = State()) {

        private var next : LBSWNode? = null
        private var prev : LBSWNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLBSWNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit)  : LBSWNode {
            var curr : LBSWNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineBiSinWave(var i : Int) {

        private var curr : LBSWNode = LBSWNode(0)
    ...
    data class Renderer(var view : LineBiSineWaveView) {

        private val lbsw : LineBiSinWave = LineBiSinWave(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val animator : Animator = Animator(view)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            lbsw.draw(canvas, paint)
            animator.animate {
                lbsw.update {
                    animator.stop()
                }

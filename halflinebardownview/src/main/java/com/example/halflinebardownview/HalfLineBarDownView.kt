...
class HalfLineBarDownView(ctx : Context) : View(ctx) {
    ...
    data class HLBDNode(var i : Int, val state : State = State()) {

        private var next : HLBDNode? = null
        private var prev : HLBDNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawHLBDNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : HLBDNode {
            var curr : HLBDNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class HalfLineBarDown(var i : Int) {

        private var curr : HLBDNode = HLBDNode(0)

...
class CircleBiLineJoinDownView(ctx : Context) : View(ctx) {
    ...
    data class CBLJDNode(var i : Int = 0, val state : State = State()) {

        private var next : CBLJDNode? = null
        private var prev : CBLJDNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawCBLJDNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : CBLJDNode {
            var curr : CBLJDNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class CircleBiLineJoinDown(var i : Int) {

        private var curr : CBLJDNode = CBLJDNode(0)

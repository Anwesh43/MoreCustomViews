...
class JoinBarTRotUpView(ctx : Context) : View(ctx) {
    ...
    data class JBTRUNode(var i : Int, val state : State = State()) {

        private var prev : JBTRUNode? = null
        private var next : JBTRUNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawJBTRUNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : JBTRUNode {
            var curr : JBTRUNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class JoinBarTRotUp(var i : Int) {

        private var curr : JBTRUNode = JBTRUNode(0)

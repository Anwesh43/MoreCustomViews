...
class OneEightyShooterView(ctx : Context) : View(ctx) {
    ...
    data class OESNode(var i : Int, val state : State = State()) {

        private var next : OESNode? = null
        private var prev : OESNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawOESNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : OESNode {
            var curr : OESNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class OneEightyShooter(var i : Int) {

        private var curr : OESNode = OESNode(0)

...
class BranchDivideFromLineView(ctx : Context) : View(ctx) {
    ...
    data class BDFLNode(var i : Int, val state : State = State()) {

        private var next : BDFLNode? = null
        private var prev : BDFLNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBDFLNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BDFLNode {
            var curr : BDFLNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BranchDivideFromLine(var i : Int) {

        private var curr : BDFLNode = BDFLNode(0)
    ...
    data class Renderer(var view : BranchDivideFromLineView) {

        private val bdfl : BranchDivideFromLine = BranchDivideFromLine(0)
        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            bdfl.draw(canvas, paint)
            animator.animate {
                bdfl.update {
                    animator.stop()
                }

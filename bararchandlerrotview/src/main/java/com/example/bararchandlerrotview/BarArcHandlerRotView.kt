...
class BarArcHandlerRotView(ctx : Context) : View(ctx) {
    ...
    data class BAHRNode(var i : Int = 0, val state : State = State()) {

        private var prev : BAHRNode? = null
        private var next : BAHRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBAHNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BAHRNode {
            var curr : BAHRNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BarArcHandlerRot(var i : Int) {

        private var curr : BAHRNode = BAHRNode(0)

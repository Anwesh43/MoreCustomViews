...
class BarRisingLineRightView(ctx : Context) : View(ctx) {
    ...
    data class BRLRNode(var i : Int = 0, val state : State = State()) {

        private var next : BRLRNode? = null
        private var prev : BRLRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBRLRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BRLRNode {
            var curr : BRLRNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BarRisingLineRight(var i : Int) {

        private var curr : BRLRNode = BRLRNode(0)

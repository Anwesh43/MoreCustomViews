...
fun Canvas.drawSemiCircleRot(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 + (h / 2) * dsc(2)) {
        rotate(rot * dsc(1))
        drawArc(RectF(-size / 2, -size / 2, size / 2, size / 2), rot, rot * dsc(0), false, paint)
    }
}

fun Canvas.drawSCRNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawSemiCircleRot(scale, w, h, paint)
}
...
class SemiCircleRotDownView(ctx : Context) : View(ctx) {
    ...
    data class SCRDNode(var i : Int = 0, val state : State = State()) {
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawSCRNode(i, state.scale, paint)
        }

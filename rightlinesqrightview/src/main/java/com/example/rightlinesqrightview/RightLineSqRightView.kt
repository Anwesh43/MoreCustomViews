...
class RightLineSqRightView(ctx : Context) : View(ctx) {
    ...
    data class RLSRNode(var i : Int = 0, val state : State = State()) {

        private var next : RLSRNode? = null
        private var prev : RLSRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawRLSRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : RLSRNode {
            var curr : RLSRNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class RightLineSqRight(var i : Int) {

        private var curr : RLSRNode = RLSRNode(0)

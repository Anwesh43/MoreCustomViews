package com.example.bisidesqsizeview
...
fun Canvas.drawBiSideSqSize(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2 + size) * dsc(3), h / 2) {
        ...
        for (j in 0..1) {
            drawXY(0f, 0f) {
                scale(1f - 2 * j, 1f)
                drawXY(size / 6, 0f) {
                    drawLineWithoutDot(0f, 0f, 0f, -size * 0.25f * (1 + j) * dsc(0), paint)
...
class BiSideSqSizeView(ctx : Context) : View(ctx) {
    ...
    data class BiSideSqSize(var i : Int) {
    ...
    data class Renderer(var view : BiSideSqSizeView) {

        private val animator : Animator = Animator(view)
        private val bsss : BiSideSqSize = BiSideSqSize(0)

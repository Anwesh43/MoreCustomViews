package com.example.trotlinerightview
...
fun Canvas.drawTRLRNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawTRotLineRight(scale, w, h, paint)
}

class TRotLineRightView(ctx : Context) : View(ctx) {
    ...
    data class TRotLineRight(var i : Int) {

        private var curr : TRLRNode = TRLRNode(0)

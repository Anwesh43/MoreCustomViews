package com.example.paralleltorightlineview
...
fun Canvas.drawPTRLNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawParallelToRightLine(scale, w, h, paint)
}

class ParallelToRightLineView(ctx : Context) : View(ctx) {
    ...
    data class ParallelToRightLine(var i : Int) {

        private var curr : PTRLNode = PTRLNode(0)

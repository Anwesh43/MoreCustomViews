...
class HalfArcAboveSqView(ctx : Context) : View(ctx) {
    ...
    data class HAASNode(var i : Int, val state : State = State()) {

        private var next : HAASNode? = null
        private var prev : HAASNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawHAASNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : HAASNode {
            var curr : HAASNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class HalfArcAboveSq(var i : Int) {

        private var curr : HAASNode = HAASNode(0)

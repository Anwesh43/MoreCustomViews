...
class BallAboveBoxRotView(ctx : Context) : View(ctx) {
    ...
    data class BABRNode(var i : Int = 0, val state : State = State()) {

        private var next : BABRNode? = null
        private var prev : BABRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBABRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BABRNode {
            var curr : BABRNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {

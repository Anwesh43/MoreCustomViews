...
fun Canvas.drawTriJoinRight(scale : Float, w : Float, h : Float, paint : Paint) {
    val dsc : (Int) -> Float = {
...
fun Canvas.drawTJRNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawTriJoinRight(scale, w , h, paint)
}
...
class TriJoinArcRightView(ctx : Context) : View(ctx) {
    ...
    data class TJARNode(var i : Int = 0, val state : State = State()) {
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawTJRNode(i, state.scale, paint)
        }
    ...
    data class Renderer(var view : TriJoinArcRightView) {

        private val animator : Animator = Animator(view)
        private val traj : TriJoinArcRight = TriJoinArcRight(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            traj.draw(canvas, paint)
            animator.animate {
                traj.update {
                    animator.stop()
                }

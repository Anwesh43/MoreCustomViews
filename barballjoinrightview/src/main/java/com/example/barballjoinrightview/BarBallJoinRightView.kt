...
class BarBallJoinRightView(ctx : Context) : View(ctx) {
    ...
    data class BarBallJoinRight(var i : Int) {

        private var curr : BBJRNode = BBJRNode(0)
    ...
    data class Renderer(var view : BarBallJoinRightView) {

        private val bbjr : BarBallJoinRight = BarBallJoinRight(0)
        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            bbjr.draw(canvas, paint)
            animator.animate {
                bbjr.update {
                    animator.stop()
                }

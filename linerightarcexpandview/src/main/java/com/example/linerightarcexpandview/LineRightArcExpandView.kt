...
class LineRightArcExpandView(ctx : Context) : View(ctx) {
    ...
    data class LRAENode(var i : Int = 0, val state : State = State()) {
        private var prev : LRAENode? = null
        private var next : LRAENode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLRAENode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LRAENode {
            var curr : LRAENode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineRightArcExpand(var i : Int) {

        private var curr : LRAENode = LRAENode(0)

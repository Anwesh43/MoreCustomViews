package com.example.lineexpandbardownview
...
fun Canvas.drawLineExpandBarDown(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 + (h / 2) * dsc(3)) {
        ...
        for (j in 0..1) {
            save()
            scale(1f - 2 * j, 1f)
            drawXY((w / 2 + paint.strokeWidth) * (1 - dsc(0)) + size * 0.5f * dsc(1), 0f) {
...
fun Canvas.drawLEBDNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineExpandBarDown(scale, w, h, paint)
}

class LineExpandBarDownView(ctx : Context) : View(ctx) {
    ...
    data class LineExpandBarDown(var i : Int) {

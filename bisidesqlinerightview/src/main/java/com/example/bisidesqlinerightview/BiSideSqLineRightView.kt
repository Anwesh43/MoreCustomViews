...
class BiSideSqLineRightView(ctx : Context) : View(ctx) {
    ...
    data class BSSLRNode(var i : Int, val state : State = State()) {

        private var next : BSSLRNode? = null
        private var prev : BSSLRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBSSLRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BSSLRNode {
            var curr : BSSLRNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BiSideSqLineRight(var i : Int) {

        private var curr : BSSLRNode = BSSLRNode(0)

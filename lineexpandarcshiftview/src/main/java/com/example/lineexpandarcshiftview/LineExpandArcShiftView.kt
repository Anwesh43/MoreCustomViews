...
fun Canvas.drawLineExpandArcShift(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    restore()
    drawArc(RectF(-r, -r, r, r), rot + rot * sc3, rot * (sc2 - sc3), false, paint)
    restore()
}
...
class LineExpandArcShiftView(ctx : Context) : View(ctx) {
    ...
    data class LEASNode(var i : Int, val state : State = State()) {

        private var next : LEASNode? = null
        private var prev : LEASNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLEASNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LEASNode {
            var curr : LEASNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineExpandArcShift(var i : Int) {

        private var curr : LEASNode = LEASNode(0)

...
class CrossLineExtenderRotView(ctx : Context) : View(ctx) {
    ...
    data class CLERNode(var i : Int, val state : State = State()) {

        private var prev : CLERNode? = null
        private var next : CLERNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawCLERNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : CLERNode {
            var curr : CLERNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class CrossLineExtenderRot(var i : Int) {

        private var curr : CLERNode = CLERNode(0)

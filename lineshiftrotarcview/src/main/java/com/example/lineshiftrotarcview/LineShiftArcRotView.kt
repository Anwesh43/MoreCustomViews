package com.example.lineshiftrotarcview
...
fun Canvas.drawLineShiftArcRot(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 - (h / 2 + size) * dsc(3)) {
        ...
        drawArc(RectF(-size, -size, size, size), start, rot * dsc(1), true, paint)
    }
}
...
fun Canvas.drawLSARNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineShiftArcRot(scale, w, h, paint)
}

class LineShiftArcRotView(ctx : Context) : View(ctx) {
    ...
    data class LineShiftArcRot(var i : Int = 0) {
    ...
    data class Renderer(var view : LineShiftArcRotView) {

        private val animator : Animator = Animator(view)
        private val lsar : LineShiftArcRot = LineShiftArcRot(0)

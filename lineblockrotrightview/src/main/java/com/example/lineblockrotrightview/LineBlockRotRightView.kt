package com.example.lineblockrotrightview
...
fun Canvas.drawLineBlockRotRight(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2 + size) * dsc(4), h / 2) {
        ...
        drawRect(RectF(-size * dsc(3), 0f, 0f, size), paint)
    }
}
...
fun Canvas.drawLBRRNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineBlockRotRight(scale, w, h, paint)
}

class LineBlockRotRightView(ctx : Context) : View(ctx) {
    ...
    data class LineBlockBoxRotRight(var i : Int) {

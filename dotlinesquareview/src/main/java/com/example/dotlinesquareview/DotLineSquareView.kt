...
class DotLineSquareView(ctx : Context) : View(ctx) {
    ...
    data class DLSNode(var i : Int = 0, val state : State = State()) {

        private var next : DLSNode? = null
        private var prev : DLSNode? = null
        ...
        fun getNext(dir : Int, cb : () -> Unit) : DLSNode {
            var curr : DLSNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class DotLineSquare(var i : Int) {

        private var curr : DLSNode = DLSNode(0)

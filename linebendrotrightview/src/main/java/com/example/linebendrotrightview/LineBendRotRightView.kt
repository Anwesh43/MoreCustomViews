package com.example.linebendrotrightview
...
fun Canvas.drawLineBendRotRight(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2) * dsc(4), h / 2) {
        ...
        drawXY(0f, h * 0.5f * (1 - dsc(2))) {
            ...
            drawXY(0f, -size) {
                drawLineWithoutDot(0f, 0f, size * 0.33f * dsc(1), 0f, paint)
            }
        }
    }
}
...
fun Canvas.drawLBRRNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineBendRotRight(scale, w, h, paint)
}

class LineBendRotRightView(ctx : Context) : View(ctx) {
    ...
    data class LineBendRotRight(var i : Int) {

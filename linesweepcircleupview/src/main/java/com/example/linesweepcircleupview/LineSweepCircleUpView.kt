...
class LineSweepCircleUpView(ctx : Context) : View(ctx) {
    ...
    data class LSCUNode(var i : Int = 0, val state : State = State()) {

        private var next : LSCUNode? = null
        private var prev : LSCUNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLSCUNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LSCUNode {
            var curr : LSCUNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineSweepCircleUp(var i : Int) {

        private var curr : LSCUNode = LSCUNode(0)
